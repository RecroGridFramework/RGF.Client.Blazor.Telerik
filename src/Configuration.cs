...
{
    public static async Task InitializeRgfTelerikUIAsync(this IServiceProvider serviceProvider, string themeName = "kendo-theme-default/all", bool loadResources = true)
    {
    ...
    public static async Task LoadResourcesAsync(IJSRuntime jsRuntime, string themeName)
    {
        ...
        await jsRuntime.InvokeVoidAsync("import", $"{RgfClientConfiguration.AppRootPath}/_content/Telerik.UI.for.Blazor/js/telerik-blazor.js");
        await jsRuntime.InvokeVoidAsync("eval", "document.getElementsByTagName('body')[0].classList.add('k-body');");
        await jsRuntime.InvokeVoidAsync("Recrovit.LPUtils.AddStyleSheetLink", $"{RgfClientConfiguration.AppRootPath}/_content/Telerik.UI.for.Blazor/css/{themeName}.css", false, TelerikThemeId);
